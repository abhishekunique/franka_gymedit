...
class RobotEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if goal_state == 'left_open':
            ...
            self._goal_state = [1, 1, 1, -1]
        elif goal_state == 'custom':
            self._goal_state = [0.59, -0.29, 0.15, -1]
        ...
        self.observation_space = Box(-1., 1., (self.ee_space.shape[0] + self.qpos_space.shape[0],), np.float32)
        ...
        if self.sim:
            ...
            self._robot = FrankaMujocoEnv(has_renderer=has_renderer, has_offscreen_renderer=has_offscreen_renderer, control_hz=self.hz)
    ...
    def step(self, action):
        ...
        reward = -np.linalg.norm(obs['lowdim_ee'][:3] - self._goal_state[:3]) if self._goal_state is not None else 0.
    ...
    def normalize_ee_obs(self, obs):
        ...
        if self._normalize_obs:
            normalized_obs = obs # 2 * (obs - self.ee_space.low) / (self.ee_space.high - self.ee_space.low) - 1
